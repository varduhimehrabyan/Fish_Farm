...
router.get("/getPartnerDetails", tokenVerify, async (req, res) => {
  try {
    let partners = [];
    const partnersDetails = await pool.query(
      pgFunctions.partner.usp_getPartners
    );
    for (let i = 0; i < partnersDetails.rows.length; i++) {
      partners.push({
        id: partnersDetails.rows[i].id,
        name: partnersDetails.rows[i].name,
      });
    }
...
router.post("/addPartner", tokenVerify, async (req, res) => {
  try {
    ...
    const added = await pool.query(pgFunctions.partner.usp_addPartner, [
      name,
      description,
      phone,
    ]);
...
router.post("/deletePartner", tokenVerify, async (req, res) => {
  try {
    const { id } = req.body;
    const result = await pool.query(pgFunctions.partner.usp_deletePartner, [
      id,
    ]);
    res.status(200).send({
...
router.post("/updatePartner", tokenVerify, async (req, res) => {
  try {
    ...
    const result = await pool.query(pgFunctions.partner.usp_updatePartner, [
      id,
      name,
      description,
      phone,
