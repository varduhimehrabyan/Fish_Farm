...
router.post("/addPool", tokenVerify, async (req, res) => {
  try {
    ...
    const added = await pool.query(pgFunctions.pool.usp_addPool, [
      name,
      fishName,
    ]);
    res.status(200).send({
...
router.post("/updatePool", tokenVerify, async (req, res) => {
  try {
    ...
    const result = await pool.query(pgFunctions.pool.usp_updatePool, [
      id,
      name,
      fishName,
    ]);
...
router.post("/correct", tokenVerify, async (req, res) => {
  try {
    ...
    const added = await pool.query(pgFunctions.pool.usp_correct, [
      poolid,
      quantity,
      weight,
    ]);
...
router.post("/inPool", tokenVerify, async (req, res) => {
  try {
    const { toPoolid, quantity, weight, partnerId, description, date } =
      req.body;
    const result = await pool.query(pgFunctions.pool.entrance.usp_fishIn, [
      toPoolid,
      quantity,
      weight,
      partnerId,
...
router.post("/movement", tokenVerify, async (req, res) => {
  try {
    const { fromPoolid, toPoolid, quantity, weight, description, date } =
      req.body;
    const result = await pool.query(pgFunctions.pool.movement.usp_fishMove, [
      fromPoolid,
      toPoolid,
      quantity,
      weight,
...
router.post("/undoFishMove", tokenVerify, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      pgFunctions.pool.movement.usp_undoFishMove,
      [id, action]
    );
    res.send({
      success: result.rows[0].success,
...
router.post("/undoFishMoveReport", tokenVerify, async (req, res) => {
  try {
    const { id, action } = req.body;
    // console.log({ id, action });
    const result = await pool.query(
...
router.post("/sales", tokenVerify, async (req, res) => {
  try {
    const { fromPoolid, quantity, weight, partnerId, description, date } =
      req.body;
    const result = await pool.query(pgFunctions.pool.sale.usp_fishOut, [
      fromPoolid,
      quantity,
      weight,
      partnerId,
